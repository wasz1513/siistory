		...
		$(document).on(
				...
				function() {
					var writer = "@" + $(this).parents(".r").data("writer")
							+ " ";
					$(this).parents(".mb-3").next("section").find(
							".reply_content").val(writer).focus();
					var replyseq = $(this).parents(".r").data("seq");
					$(this).parents(".mb-3").next("section").find(
							".reply_content").data("replyseq", replyseq);
				});
		...
		$(".submit")
				.click(
						function(e) {
							...
							var boardseq = $(this).parents("section").prev(
									".mb-3").data("seq");
							var replyseq = $(this).prev(".reply_content").data(
									"replyseq");

							$
									.ajax({
										url : "${pageContext.request.contextPath}/dashboard/replyinsert",
										type : "post",
										data : {
											'board_no' : boardseq,
											'super_no' : replyseq,
											...
											if (data.depth == 0) {
												...
												$(this)
														...
														.find(
																"#"
																		+ data.super_no)
														.after(commentadd(data));
												$(this).prev(".reply_content")
														.data("replyseq", 0);
											}
		...
		$(".replymore")
				.click(
						function() {
							...
							$
									.ajax({
										url : "${pageContext.request.contextPath}/dashboard/morereply",
										type : "get",
										data : {
											'board_no' : board_no,
											'start' : (count * 10) + 1,
		...
		$(document)
				.on(
						...
						function() {
							...
							$
									.ajax({
										url : "${pageContext.request.contextPath}/dashboard/commentview?super_no="
												+ super_no
												+ "&board_no="
												+ board_no,
										type : "get",
...
//		//옵션 선택에 따른 db 처리 
	...
	$(document).on("change",".private_option", function(event){
		console.log("옵션 설정 적용 이벤트")
		
// 		var value = $(this).selectedIndex.value();
...
// 		$("#셀렉트ID option:selected").val();
		...
		$.ajax({
			url:"${pageContext.request.contextPath}/dashboard/private",
			type:"post",
			dataType:"JSON",
			data:{'board_no':board_no,'board_state':board_state},
			context:this
// 			success:function(data){				
// 				/* if(data.board_state==this.val()){					
// 				}; */				
// 			}			
		});				
	});
...
// 		html += '<div><img src="${pageContext.request.contextPath }/util/download?member_no='
				+ data.writer_no + '" width="20" height="20"></div>';
		html += '<div>';
...
<div>
	<form action="write" method="post">
		<input type="text" name="board_content"> <input type="submit">
	</form>
</div>

<div>${sessionScope.member_no }</div>
<DIV>${pageContext.request.contextPath}</DIV>
<!-- 컨텐츠는 결과적으로 dtolist 를 내것만 가져오면 된다 .  -->
				...
				<div>
					<form>
						<div class="form-group">
							<textarea class="form-control reply_content" rows="1"
